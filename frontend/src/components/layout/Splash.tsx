...
export default function Splash({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className={`splash ${leaving ? 'splash-leaving' : ''}`} role="status" aria-label="Loading EduConnect">
      ...
      <div className="splash-actions">
        <Link href={isAuthed ? '/dashboard' : '/auth'} className="btn btn-primary splash-cta" onClick={finish}>
          Get Started <span className="splash-arrow">→</span>
        </Link>
        <p className="splash-login">
          Already have an account? <Link href="/auth" onClick={finish}>Log in</Link>
        </p>
      </div>
