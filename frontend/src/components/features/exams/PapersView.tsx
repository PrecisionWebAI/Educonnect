...
const statusTone: Record<PaperDraftFull["status"], BadgeTone> = {
    Draft: "amber",
    Submitted: "accent",
    Approved: "green",
};
...
export default function PapersView({
    ...
    papers: PaperDraftFull[];
    reviews: PaperReviewItem[];
}) {
    const cols = [
        ...
        { key: "totalMarks", header: "Marks" },
        {
            key: "status",
            header: "Status",
            render: (r: PaperDraftFull) => <Badge tone={statusTone[r.status]}>{r.status}</Badge>,
        },
