...
export default function CopilotPage() {
    ...
    return (
        <div>
            <PageHeader
                title="AI Copilot"
                subtitle="Your school assistant for insights, drafts and automation."
            />
                    ...
                    {c.tab === "Command Palette" && (
                        <>
                            <p style={{ color: "var(--muted)", margin: "16px 0 8px" }}>
                                Press <b>Ctrl-K</b> anywhere in the app to open the palette. Top
                                commands:
                            </p>
                            <Table
                                columns={[
                                    {
                                        key: "label",
                                        header: "Command",
                                        render: (x: PaletteCommand) => <b>{x.label}</b>,
                                    },
                                    {
                                        key: "shortcut",
                                        header: "Shortcut",
                                        render: (x: PaletteCommand) => (
                                            <Badge tone="accent">{x.shortcut}</Badge>
                                        ),
                                    },
