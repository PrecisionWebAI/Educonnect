...
export default function StudentFormModal({
  ...
  useEffect(() => {
    ...
    setForm(
      editing
        ? {
            name: editing.name, admissionNo: editing.admissionNo, className: editing.className,
            section: editing.section, gender: editing.gender, guardian: editing.guardian,
            phone: editing.phone, email: editing.email,
          }
        : { ...EMPTY_FORM, admissionNo: defaultAdmissionNo },
    )
  }, [open, editing, defaultAdmissionNo])
  ...
  return (
    <Modal open={open} title={editing ? `Edit — ${editing.name}` : 'Add new student'} onClose={onClose}>
