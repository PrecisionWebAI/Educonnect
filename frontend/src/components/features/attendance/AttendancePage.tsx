...
export default function AttendancePage() {
  ...
  return (
    <div className="page">
      ...
      <Tabs tabs={['Mark attendance', 'History', 'Irregularity & Insights', 'Leave Sync']} active={tab} onChange={setTab} />
      ...
      {att.loading ? (
        ...
        <Table
          columns={[
            { key: 'name', header: 'Student', render: (r: IrregularStudent) => <b>{r.name}</b> },
            { key: 'className', header: 'Class' },
            { key: 'absences', header: 'Absences (term)' },
            { key: 'pattern', header: 'AI pattern insight' },
            { key: 'risk', header: 'Risk', render: (r: IrregularStudent) => <Badge tone={r.risk === 'High' ? 'red' : r.risk === 'Medium' ? 'amber' : 'green'}>{r.risk}</Badge> },
          ]}
          rows={irregular}
          rowKey={(r) => r.id}
        ...
        <Table
          columns={[
            { key: 'student', header: 'Student', render: (r: LeaveSyncRow) => <b>{r.student}</b> },
            { key: 'className', header: 'Class' },
            { key: 'from', header: 'From' },
            { key: 'days', header: 'Days' },
            { key: 'autoMarked', header: 'Attendance auto-marked as' },
            { key: 'status', header: 'Sync status', render: (r: LeaveSyncRow) => <Badge tone={r.status === 'Synced' ? 'green' : r.status === 'Pending' ? 'amber' : 'muted'}>{r.status}</Badge> },
          ]}
          rows={leaveSync}
          rowKey={(r) => r.id}
