...
export default function ClassTeacherDashboard() {
  ...
  useEffect(() => {
    void Promise.all([getTodayClasses(), getHomeworkStatus(), getSubjectLeaves(), getMeetingReminders()]).then(([c, h, l, m]) => {
      setClasses(c); setHw(h); setLeaves(l); setMeetings(m); setReady(true)
    })
  }, [])
            ...
            <ul className="feed">
              {leaves.map((l) => (
                <li key={l.id} className="feed-item">
                  ...
                  <Button size="sm" variant="success" onClick={() => { setLeaves((x) => x.filter((y) => y.id !== l.id)); toast.push('success', 'Leave approved') }}>Approve</Button>
                </li>
              ))}
            </ul>
          )}
