...
import { getStudentPortal } from "@/services";
...
export default function StudentDashboard() {
    ...
    useEffect(() => {
        void getStudentPortal().then(setData);
    }, []);
    ...
    const perf = [
        ...
        { icon: "groups" as const, label: "Class Rank", value: `#${data.rank}` },
        {
            icon: "book" as const,
            label: "Pending HW",
            value: String(data.homework.filter((h) => !h.done).length),
        },
    ];
    ...
    return (
        <div className="page">
            <PageHeader
                title={`Hi, ${data.name}`}
                subtitle={`Class ${data.className}-${data.section}`}
            />
                    ...
                    <ul className="feed">
                        {data.homework.map((h) => (
                            <li key={h.id} className="feed-item">
                                <Badge
                                    tone={h.done ? "green" : h.due === "Today" ? "red" : "amber"}
                                >
                                    {h.done ? "Done" : h.due}
                                </Badge>
                                <div className="feed-body">
