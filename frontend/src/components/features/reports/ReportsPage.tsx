...
const qTone: Record<DataQualityRow["status"], BadgeTone> = {
    Healthy: "green",
    Attention: "amber",
    Critical: "red",
};
...
export default function ReportsPage() {
    ...
    const qualityCols = [
        ...
        { key: "issue", header: "Open issue" },
        {
            key: "status",
            header: "Status",
            render: (row: DataQualityRow) => <Badge tone={qTone[row.status]}>{row.status}</Badge>,
        },
    ];
                ...
                <>
                        ...
                        <Card title="Build a custom report">
                            ...
                            <div className="modal-actions">
                                <Button
                                    ...
                                    onClick={() =>
                                        alert(
                                            "Report generated (mock) — backend will stream Excel/PDF.",
                                        )
                                    }
                                >
                                    Generate
                                ...
                                <Button
                                    variant="outline"
                                    onClick={() =>
                                        alert("Scheduled — will email every Monday (mock).")
                                    }
                                >
                                    Schedule weekly
                                </Button>
                            ...
                            columns={[
                                {
                                    key: "metric",
                                    header: "Metric",
                                    render: (row: EducationReportRow) => <b>{row.metric}</b>,
                                },
                                ...
                                { key: "value", header: "Value" },
                                {
                                    key: "trend",
                                    header: "Trend",
                                    render: (row: EducationReportRow) => (
                                        <Badge tone={row.trend.includes("-") ? "amber" : "green"}>
                                            {row.trend}
                                        </Badge>
                                    ),
                                },
                    ...
                    {tab === "Data Quality" && (
                        <Table
                            ...
                            rows={r.quality}
                            rowKey={(row) => row.id}
                            empty="No data quality issues found."
                        />
                    )}
                </>
            )}
