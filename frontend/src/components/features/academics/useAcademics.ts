...
export function gradeFor(percent: number): { label: string; tone: 'green' | 'teal' | 'amber' | 'red' | 'muted' } {
...
export function useAcademics() {
  ...
  const exams = useMemo(
    () => Array.from(new Set((entries ?? []).map((e) => e.exam))),
    [entries],
  )
      ...
      (prev) =>
        (prev ?? []).map((e) =>
          e.studentId === studentId
            ? { ...e, rows: e.rows.map((r) => (r.subject === subject ? { ...r, obtained } : r)) }
            : e,
        ),
    )
  }
  ...
  return {
    ...
    entries: filtered,
    exams, exam, setExam,
    classNames, className, setClassName,
    subjects,
    updateScore,
  }
}