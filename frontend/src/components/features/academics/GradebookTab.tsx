...
export default function GradebookTab({ entries }: { entries: MarksEntry[] }) {
  const columns: Column<MarksEntry>[] = [
    {
      ...
      render: (e) => (
        <span>
          ...
          <br />
          <span style={{ color: 'var(--muted)', fontSize: '0.78rem' }}>{e.className} · {e.exam}</span>
        </span>
      ),
    },
    ...Object.keys(
      entries.reduce<Record<string, true>>((acc, e) => {
        e.rows.forEach((r) => { acc[r.subject] = true })
        return acc
      }, {}),
    ).map((subject) => ({
      key: subject,
      ...
      render: (e: MarksEntry) => {
        ...
        return <span>{row.obtained}/{row.max} <small style={{ color: 'var(--muted)' }}>({pct}%)</small></span>
      },
    })),
    {
      key: 'percent',
  ...
  return <Table columns={columns} rows={entries} rowKey={(e) => e.studentId} empty="No marks records yet." />
}