      ...
      {s.loading ? (
        ...
        <>
          <div className="stat-tiles">
            <div className="stat-tile"><b>{s.staff.length}</b><span>Total Staff</span></div>
            <div className="stat-tile"><b>{s.activeCount}</b><span>Active</span></div>
            <div className="stat-tile"><b>{s.leaveCount}</b><span>On Leave</span></div>
            <div className="stat-tile"><b>{s.matrix.reduce((a, r) => a + r.periods, 0)}</b><span>Periods / wk</span></div>
          </div>

          <Tabs
            tabs={TABS}
            active={tab}
            onChange={(t) => setTab(t as StaffTab)}
          />
            ...
            <>
              <div className="toolbar">
                <div className="toolbar-search">
                  <Input placeholder="Search name, subject, ID…" value={s.query} onChange={(e) => s.setQuery(e.target.value)} />
                </div>
              ...
              <StaffListTable rows={s.filtered} />
            </>
          )}
