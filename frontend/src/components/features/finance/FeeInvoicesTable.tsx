...
export const FEE_COLUMNS = [
  ...
  { key: 'due', header: 'Due', render: (r: FeeInvoice) => <b style={{ color: r.due > 0 ? '#fda4af' : 'var(--text)' }}>{inr(r.due)}</b> },
  { key: 'status', header: 'Status', render: (r: FeeInvoice) => <Badge tone={feeStatusTone(r.status)}>{r.status}</Badge> },
]
