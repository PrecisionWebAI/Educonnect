...
import { getCollectionReports } from "@/services";
    ...
    return (
        <>
            <p style={{ color: "var(--muted)", marginBottom: "0.6rem" }}>
                Collection reports — daily, weekly and term-wise; per head and per class drill-down
                ...
                columns={[
                    {
                        key: "period",
                        header: "Period",
                        render: (r: CollectionReportRow) => <b>{r.period}</b>,
                    },
                    ...
                    { key: "collected", header: "Collected" },
                    {
                        key: "variance",
                        header: "Variance",
                        render: (r: CollectionReportRow) => (
                            <Badge tone={r.variance.startsWith("-") ? "amber" : "green"}>
                                {r.variance}
                            </Badge>
                        ),
                    },
