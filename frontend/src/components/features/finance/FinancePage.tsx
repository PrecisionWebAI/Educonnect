      ...
      {f.loading ? (
        ...
        <>
          <div className="stat-tiles">
            <div className="stat-tile"><b>{inr(f.collected)}</b><span>Collected</span></div>
            <div className="stat-tile"><b>{f.invoicesCount}</b><span>Invoices</span></div>
            <div className="stat-tile"><b>{f.pendingDues}</b><span>In Dues</span></div>
            <div className="stat-tile"><b>{f.paidCount}</b><span>Fully Paid</span></div>
          </div>
            ...
            <>
              <div className="toolbar">
                <div className="toolbar-search">
                  <Input placeholder="Search student, class, fee head…" value={f.query} onChange={(e) => f.setQuery(e.target.value)} />
                </div>
              ...
              {tab === 'Dues & Recovery' && (
                <p style={{ color: 'var(--muted)', marginBottom: '0.6rem' }}>
                  Outstanding dues: <b style={{ color: '#fda4af' }}>{inr(totalDues)}</b>
                </p>
              )}
              <FeeInvoicesTable rows={tab === 'Dues & Recovery' ? dues : f.filtered} />
            </>
          )}
