...
export default function SalaryStructureView({ structures }: { structures: SalaryStructureRow[] }) {
  const columns = [
    { key: 'staffCode', header: 'ID', render: (r: SalaryStructureRow) => <span style={{ color: 'var(--muted)', fontSize: '0.8rem' }}>{r.staffCode}</span> },
    ...
    { key: 'special', header: 'Special', render: (r: SalaryStructureRow) => currency(r.special) },
    { key: 'total', header: 'Total', render: (r: SalaryStructureRow) => <b>{currency(r.total)}</b> },
  ]
  return (
    <Table
      columns={columns}
