...
export default function BusesTable({ rows }: { rows: Bus[] }) {
    ...
    const columns = [
            ...
            render: (r: Bus) => (
                ...
                >
                    <div className="cls-bar" style={{ flex: 1, height: 8, maxWidth: 100 }}>
                        <div
                            className="bar-fill"
                            style={{ width: `${Math.round((r.occupied / r.capacity) * 100)}%` }}
                        />
                    </div>
        ...
        },
        {
            key: "status",
            header: "Status",
            render: (r: Bus) => <Badge tone={tone[r.status]}>{r.status}</Badge>,
        },
    ];
    return <Table columns={columns} rows={rows} rowKey={(r) => r.id} empty="No buses available." />;
}
